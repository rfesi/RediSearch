...
#define STRINGIFY(s) #s
  ...
  int limitArgPos = 0;
  long long offsetLimit = 0;
  long long numberLimit = LONG_MAX;
  if ((limitArgPos = RMUtil_ArgExists("LIMIT", argv, argc, 0))) {
    if (limitArgPos + 2 >= argc) {
      RedisModule_ReplyWithError(ctx, "LIMIT arg is given but no OFFSET and NUMBER come after");
      goto end;
    }
    if (RedisModule_StringToLongLong(argv[limitArgPos + 1], &offsetLimit) != REDISMODULE_OK ||
        offsetLimit < 0) {
      RedisModule_ReplyWithError(
          ctx, "bad limit offset given, offset must not be a negative number");
      goto end;
    }
    if (RedisModule_StringToLongLong(argv[limitArgPos + 2], &numberLimit) != REDISMODULE_OK ||
        offsetLimit < 0) {
      RedisModule_ReplyWithError(
